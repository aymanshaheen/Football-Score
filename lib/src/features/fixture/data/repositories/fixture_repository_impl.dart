...
class FixtureRepositoryImpl implements FixtureRepository {
  ...
  Future<Either<Failure, List<FixtureStatistics>>> getPlayersStatistics(
    ...
    if (await networkInfo.isConnected) {
      try {
        final result = await fixtureDataSource.getPlayerStatistics(fixtureId);
        List<FixtureStatistics> events = result.map((event) => event).toList();
        return Right(events);
      } on DioException catch (error) {
        return Left(ErrorHandler.handle(error).failure);
      }
  ...
  @override
  Future<Either<Failure, List<H2H>>> getH2H(String fixtureId) async {
    if (await networkInfo.isConnected) {
      try {
        final result = await fixtureDataSource.getH2H(fixtureId);
        List<H2H> events = result.map((event) => event).toList();
